...
![Image of Logo](.github/gatorgrader-example-c.png)
...
This assignment requires a programmer to implement and test a C program,
called `gators`, that will produce four lines of output. The lines of output will
contain a roster that has players' IDs, names, weights, and heights.
...
The source code in the `gators.c` file must also pass additional tests
set by the [GatorGrader tool](https://github.com/GatorEducator/gatorgrader). For
instance, GatorGrader will check to ensure that `gators.c` has the correct of
instances of the players information. When you use the `git commit` command to transfer your source
